...
#### 11. Tables: Age-specific cycling levels by state across years stratified by rural/urban ####
...
##Rural
cycle_age_r <- svyby(~ cycle, by = ~ state_name+year+age_grp, subset(svyd, sector=="rural"), 
                     svymean, vartype = c("se","ci"))
colnames(cycle_age_r)[colnames(cycle_age_r) == "cycle"] <- "mean"
cycle_age_r$mean<-round(cycle_age_r$mean*100, 1)
cycle_age_r$ci_l<-round(cycle_age_r$ci_l*100, 1)
...
cycle_age_r$ci_l<-NULL
cycle_age_r$ci_u<-NULL
cycle_age_r <- cycle_age_r %>% filter(!year==2014)

cycle_r_5to10 <- cycle_age_r %>% filter(age_grp=="5-10") %>% select(-c("age_grp")) %>% spread(year, mean)
cycle_r_11to13 <- cycle_age_r %>% filter(age_grp=="11-13") %>% select(-c("age_grp")) %>% spread(year, mean)
cycle_r_14to15 <- cycle_age_r %>% filter(age_grp=="14-15") %>% select(-c("age_grp")) %>% spread(year, mean)
cycle_r_16to17 <- cycle_age_r %>% filter(age_grp=="16-17") %>% select(-c("age_grp")) %>% spread(year, mean)

cycle_age_r_state <- cycle_r_5to10 %>% left_join(cycle_r_11to13, by="state_name")
cycle_age_r_state <- cycle_age_r_state %>% left_join(cycle_r_14to15, by="state_name")
cycle_age_r_state <- cycle_age_r_state %>% left_join(cycle_r_16to17, by="state_name")

write.csv(cycle_age_r_state, "cycle_age_r_state.csv")


##Urban
cycle_age_u <- svyby(~ cycle, by = ~ state_name+year+age_grp, subset(svyd, sector=="urban"), 
                     svymean, vartype = c("se","ci"))
colnames(cycle_age_u)[colnames(cycle_age_u) == "cycle"] <- "mean"
cycle_age_u$mean<-round(cycle_age_u$mean*100, 1)
cycle_age_u$ci_l<-round(cycle_age_u$ci_l*100, 1)
...
cycle_age_u$ci_l<-NULL
cycle_age_u$ci_u<-NULL
cycle_age_u <- cycle_age_u %>% filter(!year==2014)

cycle_u_5to10 <- cycle_age_u %>% filter(age_grp=="5-10") %>% select(-c("age_grp")) %>% spread(year, mean)
cycle_u_11to13 <- cycle_age_u %>% filter(age_grp=="11-13") %>% select(-c("age_grp")) %>% spread(year, mean)
cycle_u_14to15 <- cycle_age_u %>% filter(age_grp=="14-15") %>% select(-c("age_grp")) %>% spread(year, mean)
cycle_u_16to17 <- cycle_age_u %>% filter(age_grp=="16-17") %>% select(-c("age_grp")) %>% spread(year, mean)

cycle_age_u_state <- cycle_u_5to10 %>% left_join(cycle_u_11to13, by="state_name")
cycle_age_u_state <- cycle_age_u_state %>% left_join(cycle_u_14to15, by="state_name")
cycle_age_u_state <- cycle_age_u_state %>% left_join(cycle_u_16to17, by="state_name")

write.csv(cycle_age_u_state, "cycle_age_u_state.csv")


#### 12. GAM plots: cycling levels by age across years stratified by gender and rural/urban ####
...
#### 13. Tables: cycling levels by distance across years stratified by gender and rural/urban ####
...
#### 14. Bar plots: cycling levels by distance across years stratified by gender and rural/urban ####
...
#### 15. Box plots: Change in cycling levels and BDS implementation ####
...
#### 16. Tables: Percentage point change in cycling mode share between 2007 and 2017 relative to number of years of BDS stratified by rural/urban ####
...
#### 17. Tables: Logistic regression between likelihood of cycling and user demographics, distance across years stratified by rural/urban ####
...
#### 1. Rural: logistic regression models ####
...
#### 2. Urban: logistic regression models ####
...
#### 18. Forest plots: Logistic regression between likelihood of cycling and user demographics, distance across years stratified by rural/urban ####
...
#### 19. Tables: cycling levels by income across years stratified by gender and rural/urban ####
...
#### 20. Bar plots: cycle mode share by income quintile ####
...
#### 21. Percentage distribution of distance to school by grade ####
